...
export const analyzeFacultyData = async (
  ...
  const deptStudents = students.filter(s => s.department === faculty.department);
  const avgPerformance = deptStudents.length > 0 
    ? deptStudents.reduce((acc, s) => acc + (s.performanceScore || 0), 0) / deptStudents.length 
    : 0;

  const prompt = `
    Analyze the following faculty member's performance and contribution to the department.
    Faculty: ${faculty.name}, Role: ${faculty.facultyRole}, Dept: ${faculty.department}.
    Bio: ${faculty.bio || "N/A"}.
    Department Average Student Performance: ${avgPerformance.toFixed(2)}%.
    Number of Students in Department: ${deptStudents.length}.

    Provide a JSON response with:
    1. A short summary paragraph of their professional profile and impact.
    2. A list of 3 key strengths/contributions.
    3. A list of 3 areas for professional development or departmental improvement.
    4. A recommended action plan or goal for the next semester.
  `;
